...
namespace {
  ...
  /**
   * Write a message to the output in a view
   *
   * @param   string    $pMessage     Message to write
   * @param   boolean   $pHtmlEncode  Flag that indicates whether we should encode HTML entities
   */
  function out($pMessage, $pHtmlEncode = true) {
    if($pHtmlEncode) {
      $pMessage = Str::htmlEncode($pMessage);
    }
    echo $pMessage;
  }
}